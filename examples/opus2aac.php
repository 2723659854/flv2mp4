...
$outputFile = $argv[2] ?? __DIR__ . '/opus2aac.aac';
$bitrate = isset($argv[3]) ? (int) $argv[3] : 128000;

if (in_array($inputFile, ['-h', '--help'], true)) {
    echo '用法：php843 .\\opus2aac.php [输入Opus] [输出AAC] [码率]' . PHP_EOL;
    echo '示例：php843 .\\opus2aac.php .\\input.opus .\\opus2aac.aac 128000' . PHP_EOL;
    exit(0);
}
...
try {
    ...
    echo "使用纯 PHP 将 Ogg Opus 转码为 AAC-LC..." . PHP_EOL;
    $result = \Xiaosongshu\Flv2mp4\Client::runOpus2Aac(
        $inputFile,
        $outputFile,
        $bitrate
    );
    ...
    echo "Opus 数据包: {$result['opusPackets']}" . PHP_EOL;
