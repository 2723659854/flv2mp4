...
$server->onRtp = function (int $clientId, string $plainRtp, array $header, WebRTCServer $srv) use (&$flvRelays, $closeFlvRelay): void {
    ...
    try {
        ...
        if ($kind !== null) {
            ...
            $audioError = $relay->consumeAudioError();
            if ($audioError !== null) {
                $srv->_log_std("[ws-flv] audio disabled client={$clientId} streamId={$streamId}; continuing video-only: {$audioError}\n");
            }
            if ($relay->consumeAvcSequenceHeaderSent()) {
                $srv->_log_std("[ws-flv] AVC sequence header sent client={$clientId} streamId={$streamId}\n");
            }
            $stats = $relay->consumeStats();
            if ($stats !== null) {
                $audioAvg = $stats['audioPackets'] > 0 ? $stats['audioProcessMs'] / $stats['audioPackets'] : 0.0;
                $videoAvg = $stats['videoPackets'] > 0 ? $stats['videoProcessMs'] / $stats['videoPackets'] : 0.0;
                $srv->_log_std(sprintf(
                    "[ws-flv] stats client=%d streamId=%s elapsed=%.0fms audioRtp=%d videoRtp=%d aacTags=%d avcTags=%d audioTs=%s videoTs=%s audioProcessAvg=%.2fms audioProcessMax=%.2fms videoProcessAvg=%.2fms videoProcessMax=%.2fms\n",
                    $clientId,
                    $streamId,
                    $stats['elapsedMs'],
                    $stats['audioPackets'],
                    ...
                    $stats['aacTags'],
                    $stats['avcTags'],
                    $stats['audioTimestampMs'] === null ? '-' : (string)$stats['audioTimestampMs'],
                    $stats['videoTimestampMs'] === null ? '-' : (string)$stats['videoTimestampMs'],
                    $audioAvg,
                    $stats['maxAudioProcessMs'],
                    $videoAvg,
                    $stats['maxVideoProcessMs']
                ));
            }
        }
    } catch (\Throwable $e) {
