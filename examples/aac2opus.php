...
$outputFile = $argv[2] ?? __DIR__ . '/aac2opus.opus';
$preSkip = isset($argv[3]) ? (int) $argv[3] : 312;

if (in_array($inputFile, ['-h', '--help'], true)) {
    echo '用法：php843 .\\aac2opus.php [输入AAC/MP4/FLV] [输出OPUS] [pre-skip]' . PHP_EOL;
    echo '示例：php843 .\\aac2opus.php .\\input.aac .\\aac2opus.opus' . PHP_EOL;
    exit(0);
}
...
try {
    ...
    echo "使用纯 PHP 将 AAC-LC 音频编码为 Ogg Opus..." . PHP_EOL;
    $result = \Xiaosongshu\Flv2mp4\Client::runAac2Opus($inputFile, $outputFile, $preSkip);
    echo "Opus: {$result['output']} (" . filesize($result['output']) . " bytes)" . PHP_EOL;
    echo "参数: {$result['sampleRate']} Hz, {$result['channels']} 声道" . PHP_EOL;
    echo "AAC 帧数: {$result['frames']}，Opus 包数: {$result['packets']}" . PHP_EOL;
