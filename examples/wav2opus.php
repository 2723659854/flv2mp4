...
$inputFile = $argv[1] ?? __DIR__ . '/input.wav';
$outputFile = $argv[2] ?? __DIR__ . '/wav2opus.opus';
$preSkip = isset($argv[3]) ? (int) $argv[3] : 312;

if (in_array($inputFile, ['-h', '--help'], true)) {
    echo '用法：php843 .\\wav2opus.php [输入WAV] [输出OPUS] [pre-skip]' . PHP_EOL;
    echo '示例：php843 .\\wav2opus.php .\\input.wav .\\wav2opus.opus' . PHP_EOL;
    exit(0);
}
...
try {
    ...
    echo "使用纯 PHP 将 WAV 音频编码为 Ogg Opus..." . PHP_EOL;
    $result = \Xiaosongshu\Flv2mp4\Client::runWav2Opus($inputFile, $outputFile, $preSkip);
