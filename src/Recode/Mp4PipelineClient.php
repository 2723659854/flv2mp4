...
final class Mp4PipelineClient
{
    ...
    private function waitWorkers(): void
    {
        foreach ($this->processes as $process) {
            ...
            if ($status['running']) { proc_terminate($process); throw new RuntimeException('MP4 recode worker 结束超时'); }
            $exit = proc_close($process); if ($exit !== 0 && $exit !== -1) throw new RuntimeException("MP4 recode worker 异常退出: {$exit}");
        }
        $this->processes = [];
    }
