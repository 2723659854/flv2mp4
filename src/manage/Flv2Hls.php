...
class Flv2Hls
{
    ...
    public function write(string $flvTagData)
    {
        ...
        if ($type === 9) {
            $this->handleVideoFrameFromFile($timestamp, $payload);
        } elseif ($type === 8) {
            $this->handleAudioFrameFromFile($timestamp, $payload);
        }
    }
     ...
     * @return void
     */
    private function handleVideoFrameFromFile(int $timestamp, string $rawData): void
    {
        ...
        if ($avc['avcPacketType'] == 0) {
            $this->parseAVCDecoderConfigurationRecord($avc['data']);
            return;
        }

        if ($avc['avcPacketType'] != 1) return;

        $isKeyFrame = ($videoData['frameType'] == 1);
     ...
     * @return void
     */
    private function handleAudioFrameFromFile(int $timestamp, string $raw): void
    {
