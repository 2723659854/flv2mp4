...
class Flv2Hls1
{
    ...
    // 时间戳基准
    private  $baseTimestamp = null;      // 第一个帧的绝对时间戳
     ...
     * @param $frame
     * @return mixed|void
     * @note 此方法和xiaosongshu/rtmp_server深度绑定，不要调用
     */
    ...
    public function run(string $file)
    {
        ...
        foreach ($tags as $tag) {
            // 构建 FLV Tag 原始数据（11字节头 + body + 4字节previousTagSize）
            // type (1) + dataSize (3) + timestamp (3) + timestampExt (1) + streamId (3) = 11
            $flvTagData = chr($tag->tagType)
                . chr(($tag->dataSize >> 16) & 0xFF)
                . chr(($tag->dataSize >> 8) & 0xFF)
                . chr($tag->dataSize & 0xFF)
                . substr($tag->Timestamp, 0, 3)  // timestamp低24位
                . substr($tag->Timestamp, 3, 1) // timestamp高8位
     ...
     * @return void
     * @note 这里flv裸数据，保留此方法，作为客户端拉流转码入口
     */
